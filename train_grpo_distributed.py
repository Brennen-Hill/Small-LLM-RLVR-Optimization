"""
Distributed RLVR Training Script
...
def compute_bleu_reward(completions: List[str], references: List[str], **kwargs) -> List[float]:
    ...
    Args:
        ...
        references: List of reference solutions (ground truth sample_code)
...
def bleu_reward_function(completions, prompts=None, **kwargs):
    ...
    We need to extract the sample_code from the dataset to use as reference.
    """
    ...
    references = kwargs.get("sample_code", [""] * len(completion_texts))
...
def run_sft_warmup(
    ...
    Args:
        ...
        dataset: Processed MBPP dataset with 'prompt' and 'sample_code' columns
    ...
    # Format dataset for SFT: combine prompt and sample_code
...
def run_grpo_training(
    ...
    Args:
        ...
        dataset: Processed APPS dataset with 'prompt' and 'sample_code' columns
        output_dir: Directory to save the model
        per_device_batch_size: Training batch size per device
        local_rank: Local rank for distributed training
    ...
    # Add vLLM server host if using vLLM
    if use_vllm and vllm_server_host:
        # Parse host:port format
        if ":" in vllm_server_host:
            host, port = vllm_server_host.rsplit(":", 1)
            grpo_config.vllm_server_host = host.replace("ip-", "").replace("-", ".")
            grpo_config.vllm_server_port = int(port)
        else:
            grpo_config.vllm_server_host = vllm_server_host.replace("ip-", "").replace("-", ".")
...
def main():
    parser = argparse.ArgumentParser(description="Distributed RLVR Training")
    ...
    parser.add_argument("--vllm_server_host", type=str, default="",
                        help="vLLM server host (e.g., ip-X-X-X-X format)")
