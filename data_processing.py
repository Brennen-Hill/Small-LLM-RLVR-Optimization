...
def process_apps_dataset(sample_ratio: float = 0.01, split: str = "train") -> Dataset:
    ...
    Renames:
    - 'question' -> 'prompt'
    - 'solutions' -> 'completion' (takes first solution if multiple)
    ...
    def extract_first_solution(example):
        ...
        if isinstance(solutions, list) and len(solutions) > 0:
            return {"completion": solutions[0]}
        elif isinstance(solutions, str):
            try:
                sols = json.loads(solutions)
                if isinstance(sols, list) and len(sols) > 0:
                    return {"completion": sols[0]}
            except (json.JSONDecodeError, TypeError):
                pass
            return {"completion": solutions}
        return {"completion": ""}
    
    # Extract first solution and create completion column
    ...
    data = data.filter(lambda x: len(x["prompt"]) > 0 and len(x["completion"]) > 0)
