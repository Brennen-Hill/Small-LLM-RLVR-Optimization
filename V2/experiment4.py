...
if __name__ == "__main__":
    ...
    # Format: (Method Name, Function Reference)
    methods_to_run = [
        ("Method 1: Baseline", controller.run_method_1_baseline),
        ("Method 2: Replay", controller.run_method_2_replay),
        ("Method 3: Task Arithmetic", controller.run_method_3_task_arithmetic),
        ("Method 4: EWC Lite", controller.run_method_4_ewc_lite),
        ("Method 5: Orthogonal", controller.run_method_5_orthogonal),
        ("Method 6: Ensemble", controller.run_method_6_ensemble),
    ]

    # Iterate through each method safely
    for name, method_func in methods_to_run:
        print(f"\n{'='*20}\nSTARTING {name}\n{'='*20}")
        try:
            # Run the specific method
            method_func(ds_math, ds_logic)
            print(f">>> {name} COMPLETED SUCCESSFULLY.")
            
        except Exception as e:
            # Catch the error, log it, but DO NOT stop the loop
            print(f"\n!!! ERROR IN {name} !!!")
            print(f"Error Message: {e}")
            print("Traceback:")
            traceback.print_exc()
            print(f"!!! SKIPPING {name} AND MOVING TO NEXT METHOD !!!\n")
            
            # Optional: Record a "FAILED" result in the summary so it's not empty
            controller.results[name] = {"Math": 0.0, "Logic": 0.0}
            
            # Force memory cleanup to ensure the error didn't leave garbage on the GPU
            controller.free_memory()

    # Finally, print the summary of whatever succeeded
